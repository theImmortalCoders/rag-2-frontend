...
@Component({
  ...
  template: `<div class="flex flex-col">
    ...
    @if (collectedDataArray.length > 0 && !vIsDataCollectingActive.value) {
      <button
        (click)="generateJSON()"
        class="mt-4 text-center text-mainCreme border-mainCreme border-[1px] hover:bg-mainCreme hover:text-darkGray transition-all ease-in-out duration-300">
        Download JSON ({{ collectedDataArray.length }} records)
      </button>
      <button
        (click)="sendData()"
        class="mt-3 text-blue-900 border-blue-900 border-[1px] hover:bg-blue-900 hover:text-mainCreme transition-all ease-in-out duration-300">
        Save data
      </button>
      <button
        (click)="deleteCollectedData()"
        class="mt-3 text-center font-bold text-red-500 border-red-500 border-[1px] hover:bg-red-500 hover:text-darkGray transition-all ease-in-out duration-300">
        X
      </button>
    }
  </div>`,
